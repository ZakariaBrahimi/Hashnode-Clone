import {useRef, useState, forwardRef, useImpirative} from 'react'

const NotificationsPopup = forwardRef((props, ref) => {
  
  const [show, setShow] = useState(true)
  
  setTimeout(()=>{props.show=false}, 1000)
  console.log(props.show)
  return (props.show==true) ? (
    <div className={`fixed ${show ? 'flex' : 'hidden'} h-[3.3rem] slide-right rounded-md text-white font-medium top-[5.5rem] left-0 w-2/12 z-50 ${props.type=='success' ? 'bg-green-600' :  'bg-red-600'} justify-center items-center`}>
      <div  className='w-full px-3 relative flex gap-3 items-center justify-start w-full'>
        ...
        {props.message} 
        {/*<button className='absolute top-6 font-bold right-0'>
