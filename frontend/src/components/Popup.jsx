...
const Popup = (props) => {
  ...
  return (props.trigger) ? (
    <div onClick={closePopup}  className="fixed top-0 left-0 w-screen z-50 h-screen bg-[#00000033] flex justify-center items-center">
      <div ref={popupinner} className={`relative w-full flex flex-col gap-2 bg-white ${props.customWidth ? props.customWidth : 'max-w-[30rem]'} min-w-[10rem] m-6 p-6 rounded-xl shadow-gray-300 shadow-2xl`}>
        <button onClick={()=>props.setTrigger(false)} className={`${props.closeBTN===true && 'hidden'} p-2 rounded-full hover:bg-[#f3f4f6] absolute top-4 right-4`}>
