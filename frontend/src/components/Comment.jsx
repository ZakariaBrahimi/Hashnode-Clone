...
const Comment = ({setShowCommentInput, commentDetails, setComments})=>{
  ...
  return (
    <div key={commentDetails?.id} className=' p-4 mb-6 border rounded-lg'>
      ...
      {/* Delete a Comment Popup */}
        ...
        <Popup trigger={commentupdatePopup} setTrigger={setCommentupdatePopup}>
        <form onSubmit={comment_update} className='flex flex-col '>
            <textarea value={commentupdateContent} onChange={e=>setCommentupdateContent(e.target.value)} name='updateComment' className='w-full py-6 resize-none px-4 bg-transparent text-[#111827] min-h-[30vh] text-xl h-[75px] border-y outline outline-transparent ' placeholder="Start typing..." ></textarea>
            <button type='submit' className='py-2  w-fit px-4 rounded-full text-white bg-blue-500 font-medium self-end'>Update</button>
        </form>
